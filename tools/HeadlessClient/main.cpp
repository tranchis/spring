...
int main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		if (!fh.FileExists())
			throw content_error("Setupscript does not exists in given location: "+script);
		...
		std::cout << "Starting headless client..." << std::endl;
		...
		else
		{
			...
			if (!f->FileExists()) {
				...
				if (ars.empty()) {
					throw content_error("Could not find any archives for map '" + gameSetup->mapName + "'.");
				}
				for (std::vector<std::string>::iterator i = ars.begin(); i != ars.end(); ++i) {
					if (!vfsHandler->AddArchive(*i, false)) {
						throw content_error("Could not load archive '" + *i + "' for map '" + gameSetup->mapName + "'.");
					}
				}
			}
			delete f;
	...
	{
		std::cout << "usage: spring-headless <full_path_to_script>" << std::endl;
	}
